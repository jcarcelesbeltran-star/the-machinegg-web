(() => {
    ...
    function playSound(audioObj, vol = 1.0, baseThrottleMs = 50, rndPitch = false) {
        ...
        // 1. Voice Limit (Max 4 concurrent plays per passive sound)
        if (isPassive) {
            if (!activeVoices[key]) activeVoices[key] = 0;
            if (activeVoices[key] >= 4) return;
        }

        // 2. Dynamic Population-based Throttling
        let chickenCount = state.chickens || 1;
    ...
    function buildTextMatrix(lines) {
        ...
        for (let li = 0; li < lines.length; li++) {
            let text = lines[li];
            let rowData = [];
            for (let r = 0; r < glyphH; r++) rowData.push('');
            for (let ci = 0; ci < text.length; ci++) {
                let ch = text[ci];
                let glyph = PIXEL_FONT[ch] || PIXEL_FONT[' '];
    ...
    function createChicken() {
        ...
        return {
            ...
            eggTimer: (state.isSpeedrunMode && chickensArr.length === 0) ? 1 : Math.random() * 10,
            squishTimer: 0,
            direction: 1,
            velX: (Math.random() - 0.5) * 40,
            velY: (Math.random() - 0.5) * 40,
        ...
        if (state.money >= state.costs[item]) {
            ...
            if (item === 'chicken') {
                ...
                document.querySelectorAll('.shop-btn, .pixel-btn').forEach(b => {
                    const keep = ['wipe-btn', 'wipe-confirm', 'wipe-cancel', 'speedrun-submit-btn', 'ranking-btn', 'ranking-close', 'speedrun-start-btn', 'normal-mode-btn'];
        ...
        for (let key in shopBtns) {
            ...
            if (isMax) {
                ...
                if (btn.style.display !== 'none') {
                    btn.disabled = (key === 'retire') ? false : state.money < state.costs[key];
                }
            }
        }
            ...
            if (cinematicTimer > 6) {
                ...
                if (state.isSpeedrunMode && !localStorage.getItem('chickenIdleSpeedrunSubmitted')) {
                    ...
                    if (savedName && typeof myPin !== 'undefined' && myPin) {
                        ...
                        fetch(RANKING_API_URL + '/submit_score.php', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ name: savedName, market: GAME_MARKET, time_secs: timeSecs, chickens: chickens, pin: myPin })
                        })
                            .then(r => r.json())
                            .then(data => {
    ...
    function drawCinematic() {
        ...
        if (cinematicPhase === 0) {
            ...
            if (!window._endRankFetched) {
                ...
                fetch(RANKING_API_URL + '/get_ranking.php')
                    .then(r => r.json())
                    .then(data => {
            ...
            if (!rData) {
                ...
                for (let i = 0; i < top10.length; i++) {
                    ...
                    let mktColor = marketColors[r.market] || '#888';
                ...
                if (!playerInTop && playerPos && playerName) {
                    ...
                    let pMktColor = marketColors[playerMarket] || '#888';
                ...
                if (top10.length === 0) {
                    ctx.fillStyle = '#888';
                    ctx.textAlign = 'center';
                    ctx.fillText('No records yet', rightX + rightPanelW / 2, rY + 50);
                    ctx.fillText('Be the first!', rightX + rightPanelW / 2, rY + 70);
                }
            }
    ...
    function loop(timestamp) {
        ...
        // (Speedrun HTML timer removed in favor of canvas)
    ...
    if (pauseBtn) {
        pauseBtn.addEventListener('click', () => {
            ...
            // Optional: pause music if playing
        ...
        fetch(RANKING_API_URL + '/get_ranking.php?pin=' + encodeURIComponent(myPin))
            ...
            .then(data => {
                ...
                data.ranking.forEach(r => {
                    let isMe = r.is_me;
                    if (isMe) hasFoundSelf = true;

                    let color = posColors[r.pos - 1] || '#fff';
                    if (isMe || (data.selfRun && r.name === data.selfRun.name)) color = '#2ecc71';

                    let bgMktColor = marketColors[r.market] || '#888';
                    let mktLabel = marketLabels[r.market] || '?';
                    let badgeHTML = `<div style="background:${bgMktColor}; color:#fff; width:100px; padding:3px 0; text-align:center; font-size:6px; display:inline-block;">${mktLabel}</div>`;

                    let tr = document.createElement('tr');
                    tr.innerHTML = `<td style="text-align:center;color:${color}">${r.pos}.</td><td style="text-align:center; padding: 4px 0;">${badgeHTML}</td><td style="color:${color}">${r.name}</td><td style="text-align:right; color:#e67e22">${formatTimeSecs(r.time_secs)}</td>`;
                    tbody.appendChild(tr);
                });

                if (data.selfRun && !hasFoundSelf && data.selfRun.pos > 0) {
                    let ld = data.selfRun;
                    let trSep = document.createElement('tr');
                    trSep.innerHTML = `<td colspan="4" style="padding:10px 0;"><div style="height:2px;background:#555;width:90%;margin:0 auto;"></div></td>`;
                    tbody.appendChild(trSep);

                    let bgMktColor = marketColors[ld.market] || '#888';
                    let mktLabel = marketLabels[ld.market] || '?';
                    let badgeHTML = `<div style="background:${bgMktColor}; color:#fff; width:100px; padding:3px 0; text-align:center; font-size:6px; display:inline-block;">${mktLabel}</div>`;
                    let tr = document.createElement('tr');
                    tr.innerHTML = `<td style="text-align:center;color:#2ecc71">${ld.pos}.</td><td style="text-align:center; padding: 4px 0;">${badgeHTML}</td><td style="color:#2ecc71">${ld.name}</td><td style="text-align:right; color:#e67e22">${formatTimeSecs(ld.time_secs)}</td>`;
                    tbody.appendChild(tr);
                }
            })
    ...
    if (speedrunSubmitBtn) {
        speedrunSubmitBtn.addEventListener('click', () => {
            ...
            if (state.money + _totalMoneySpent > _totalMoneyEarned + 100) {
                speedrunSubmitBtn.innerText = "CHEATER DETECTED!";
                speedrunSubmitBtn.style.background = "#e74c3c";
                speedrunSubmitBtn.disabled = true;
                return;
            }

            speedrunSubmitBtn.innerText = "SENDING...";
            speedrunSubmitBtn.disabled = true;
            ...
            fetch(RANKING_API_URL + '/submit_score.php', {
                ...
                .then(data => {
                    if (data.ok) {
                        speedrunSubmitBtn.innerText = `#${data.pos} SUBMITTED!`;
                        localStorage.setItem('chickenIdleSpeedrunSubmitted', 'true');
                        ...
                        speedrunSubmitBtn.innerText = data.error || 'ERROR';
                    }
                    setTimeout(() => {
                ...
                .catch(() => {
                    speedrunSubmitBtn.innerText = 'OFFLINE - NOT SENT';
                    setTimeout(() => {
                        if (nameInputOverlay) nameInputOverlay.style.display = 'none';
                    }, 2000);
                });
